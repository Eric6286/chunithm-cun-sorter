...
**本程序不需要放进游戏目录。** 安装器默认装到 `%LOCALAPPDATA%\Programs\今天你寸了吗\`，
不弹 UAC；游戏在哪由你在安装向导里选一次，之后记在配置里。

| | 位置 |
|---|---|
| 程序 | `%LOCALAPPDATA%\Programs\今天你寸了吗\`（安装时可改） |
...
1. 下载最新 `chunithm-cun-sorter-2.0.2-安装程序.exe`，双击。
...
1. 下载 Release 里的 `cun_dghub_plugin_v2.0.zip`，在 DGHub「插件中心 → 外部插件 → 导入 zip 包」安装「今天你寸了吗 · 联动」并启用；触发开关 / 强度 / 波形预设 / 通道都在其插件配置页调，「启动检查」可看连接状态。
