...
**自动截图**同样吃曲终信号：随后每 0.5 秒抓一帧游戏窗口（无边框窗口直接屏幕拷贝），用**像素指纹**
判断是否已切到结算画面——17 个在全部存量结算图上恒定不变的 UI 骨架像素点（`tools/gen_result_signature.py`
从归档截图统计生成；打歌画面只命中 9/17，阈值 16/17）——命中后再等 `delay_s` 秒让分数滚完、复验一次
才落盘；判定数据（得分 / ATTACK / MISS）直接写入 OCR 缓存，归档分类零 OCR。
